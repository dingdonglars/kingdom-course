...
Read `ai-context/CLAUDE.md` for the bucket policy and the current curriculum phase. Walking through code is in the green bucket — pure learning, always fine.
...
- **Stay calibrated.** Use vocabulary at or below the learner's current curriculum phase (`ai-context/CLAUDE.md`). If a line uses a feature they haven't met yet, name the feature and say *"this is X — you'll meet it formally in module Y; for now treat it as Z"*.
