...
> Travou no inglês? Abra o `GLOSSARY.pt.md` — é este mesmo glossário em português. Tente em inglês primeiro.
...
- **AI Unlock** — the curriculum's named transition (end of Phase 3 / M4) at which AI moves from "friction-only" to "real collaborator." *Module 3.9.*
...
- **extension method** — a static method whose first parameter has `this`; looks like an instance method where you use it. LINQ is built on these. *Module 1.6.*
...
- **mode flag** — single line in `CLAUDE.md` controlling AI assistance behavior (`pre-unlock` / `post-unlock`). *Module 3.9.*
...
- **`record`** — C# keyword for a small immutable data class where two records with the same fields are equal automatically, ToString, and deconstruction. Perfect for events. *Module 1.7.*
